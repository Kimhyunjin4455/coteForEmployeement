...
# 출발 노드를 거쳐 다른 노드로 가는 비용 계산(출발 노드와 연결된 모든 간선을 하나씩 확인), 더 짧은 경로를 찾았으니 최단 거 리스트의 무한 값을 최단 거리 값으로 갱신
...
# bfs/dfs 와 마찬가지로 모든 리스트는 (노드의 갯수 +1)로 할당 -> 노드의 번호를 인덱스로 하여 바로 리스트에 접근할 수 있도록 설정(그래프 표현 시 많이 사용하는 일반적인 코드)
